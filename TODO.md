...
- [ ] **Fix broken photo credit links** — Every face in `facePhotos` has `credit: ''` and `creditUrl: ''`, causing a broken `— View source` link to appear for all users. Populate the fields or remove the credit line entirely.
...
- [ ] **Remove unused `.direction-arrow` CSS** — The class is styled but no element with that class is ever added to the DOM. Clean it up or wire it up.
...
- [ ] **Add meta tags and favicon** — No `<meta name="description">`, no Open Graph tags (`og:title`, `og:image`, `og:url`), and no favicon. Shared links have no preview. A simple Kaaba silhouette or crescent as a favicon would help.
...
- [ ] **Modal accessibility (focus trapping + ARIA)** — Modals should trap Tab focus inside while open. Both modals also need `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` pointing to their title element.
